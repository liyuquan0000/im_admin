...
//        }
    ...
    public static Date addTime(Date date, Integer secondTime) {
        Calendar c = Calendar.getInstance();
        c.setTime(date);
        c.add(Calendar.SECOND, secondTime);
        c.set(Calendar.MILLISECOND, 0);
        return c.getTime();
    }
     ...
     * @return
     */
    public static Date moveDate(Date date, int dayNum, boolean clearTime, boolean fullTime) {
     ...
     *
     * @param date      指定的时间
     * @param monthNum  移动的月数
     * @param clearTime true-清除时分秒毫秒,false-不清除
     * @return
     */
    public static Date moveMonth(Date date, int monthNum, boolean clearTime, boolean fullTime) {
        if (date == null) return null;

        Calendar c = Calendar.getInstance();
        c.setTime(date);
        int curMonth = c.get(Calendar.MONTH);
        c.set(Calendar.MONTH, curMonth + monthNum);
        if (clearTime) {
            c.set(Calendar.HOUR_OF_DAY, 0);
            c.set(Calendar.MINUTE, 0);
            c.set(Calendar.SECOND, 0);
    ...
    public static void main(String[] args) {
        System.out.println(getSampleDate(new Date()));
    }
